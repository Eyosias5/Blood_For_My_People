                ...
                if (user != null && user.password.equals(password.value)) {
                    withContext(Dispatchers.Main) {
                        val sharedPreferences =
                            _context.getSharedPreferences(Constants().currentUser, Context.MODE_PRIVATE)
                        ...
                        when {
                            ...
                            user.role.equals("Recepient") -> {
                                val recepient: Recepient? =
                                    recepientRepository.findRecepientByUserIdAsync(user.userId!!).body()
                                if (recepient != null) {
                                    with(sharedPreferences.edit()) {
        ...
        GlobalScope.launch {
            try {
                ...
                if (user != null) {
                    Toast.makeText(
                        _context,
                        "User already exists. Try a different Username/Password Combination.",
                        Toast.LENGTH_SHORT
                    ).show()
                } else {
